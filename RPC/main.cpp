...
// parser.SetStripFlag(false);
   ...
   auto* runManager = G4RunManagerFactory::CreateRunManager();
    // Mandatory user initialization classes
       if(parser.GetVolume ("Aluminium")==NULL)
        G4cout<<"Error finding Aluminium_1"<<G4endl;

   runManager->SetUserInitialization(new G01DetectorConstruction(parser.GetWorldVolume()));
   runManager->SetUserInitialization(new FTFP_BERT);
  //  runManager->SetNumberOfThreads(1);  // This line is for creating and storing data in only 1 single csv file 
